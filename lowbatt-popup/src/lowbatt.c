...
#define POWER_SAVING_UG            "setting-powersaving-efl"
...
static int load_power_saving_ug(void)
{
	int ret;
	bundle *b;
	...
	ret = aul_launch_app(POWER_SAVING_UG, b);
	bundle_free(b);
	if (ret <= 0) {
		_E("Failed to launch app (%s)(%d)", POWER_SAVING_UG, ret);
		return ret;
	}
	return 0;
}
...
static void lowbat_power_saving_selected(void *data, Evas_Object *obj, void *event_info)
{
	if (load_power_saving_ug() < 0)
		_E("Failed to load power saving ug");
	popup_terminate();
}
...
static bool is_power_saving_on(void)
{
	int val;
	...
	if (val == SETTING_PSMODE_ADVISOR /* Power saving */
			|| val == SETTING_PSMODE_SURVIVAL) /* Ultra power saving or emergency */
		return true;
...
static int load_battery_warning_popup(struct appdata *ad)
{
	char *content, *title;
	char *ltext, *rtext;
	void (*lselected)(void *data, Evas_Object *obj, void *event_info);
	void (*rselected)(void *data, Evas_Object *obj, void *event_info);
	...
	if (is_power_saving_on()) {
		ltext = _("IDS_COM_SK_OK");
		lselected = lowbatt_timeout_func;
		rtext = NULL;
		rselected = NULL;
	} else {
		ltext = _("IDS_ST_MBODY_POWER_SAVING");
		lselected = lowbat_power_saving_selected;
		rtext = _("IDS_COM_SK_OK");
		rselected = lowbatt_timeout_func;
	}
