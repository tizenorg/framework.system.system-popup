...
static int create_device_options_popup(struct appdata *ad, char *layout_style)
{
	GList *l;
	struct device_option *opt;
	...
	if (layout_style) {
		...
		elm_layout_file_set(layout, EDJ_NAME, layout_style);
...
static int app_reset(bundle *b, void *data)
{
	...
	ret = create_device_options_popup(ad, LAYOUT_DEFAULT);
	if (ret < 0) {
