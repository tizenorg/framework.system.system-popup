...
static Eina_Bool outdoor_disabled = EINA_FALSE;
...
static void unregister_edbus_signal_handler(void)
{
	...
	if (conn && outdoor_handler) {
		e_dbus_signal_handler_del(conn, outdoor_handler);
...
static int register_edbus_signal_handler(struct appdata *ad)
{
	...
	outdoor_handler = e_dbus_signal_handler_add(conn, NULL, DEVICED_PATH_DISPLAY,
...
static void low_battery_state_changed(keynode_t *key, void *data)
{
	int state;

	state = vconf_keynode_get_int(key);
	if (state == 1 && outdoor_disabled == EINA_TRUE)
		return;
	if (state == 0 && outdoor_disabled == EINA_FALSE)
		return;

	update_item();
}

static void register_low_battery_state_handler(void)
{
	int ret;
	ret = vconf_notify_key_changed(VCONFKEY_LOW_BATTERY_EXTREME,
			low_battery_state_changed, NULL);
	if (ret != 0)
		_E("Failed to notify vconf key(%d)", ret);
}

static void unregister_low_battery_state_handler(void)
{
	vconf_ignore_key_changed(VCONFKEY_LOW_BATTERY_EXTREME,
				low_battery_state_changed);
}

static Eina_Bool is_battery_extremely_low(void)
{
	int state;

	if (vconf_get_int(VCONFKEY_LOW_BATTERY_EXTREME, &state) == 0
			&& state == 1) {
		outdoor_disabled = EINA_TRUE;
	} else {
		outdoor_disabled = EINA_FALSE;
	}

	return outdoor_disabled;
}
...
static int show_outdoor_toast(struct appdata *ad)
{
	...
	snprintf(text, sizeof(text), "%s",
			_("IDS_IDLE_POP_AFTER_P1SD_MINS_BRIGHTNESS_WILL_BE_RESET_TO_DEFAULT_LEVEL_HP2SD"));
	ret = change_pd_to_ps(text);
	if (ret < 0)
		return ret;

	if (get_str_from_icu(OUTDOOR_TIMEOUT/60, time, sizeof(time)) < 0)
		snprintf(time, sizeof(time), "%d", OUTDOOR_TIMEOUT/60);
	if (get_str_from_icu(DEFAULT_BRIGHTNESS, brt, sizeof(brt)) < 0)
		snprintf(brt, sizeof(brt), "%d", DEFAULT_BRIGHTNESS);

	snprintf(content, sizeof(content), text, time, brt);
...
static void register_snd_od_handlers(void *data)
{
	...
	register_low_battery_state_handler();
}
...
static void unregister_snd_od_handlers(void *data)
{
	...
	unregister_low_battery_state_handler();
}
...
static Evas_Object *get_snd_od_icon(void *data, Evas_Object *obj, const char *part)
{
	...
	if (!strcmp(part, "elm.icon.1")) {
		...
		elm_object_disabled_set(outdoor_btn, is_battery_extremely_low());
		return outdoor_btn;
	}
...
static __attribute__ ((constructor)) void register_device_option_snd_od(void)
{
	...
	/* If power saving mode is on, sound/outdoor button is skipped */
