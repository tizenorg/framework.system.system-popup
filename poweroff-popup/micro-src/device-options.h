...
#define LAYOUT_SCROLLABLE "micro_3_title_content_button"
#else
#define LAYOUT_DEFAULT    NULL
#define LAYOUT_SCROLLABLE NULL
#endif
...
struct double_item_option {
	const char *name;
	Evas_Object *btn;
	int (*get_icon)(char *icon, int len);
	int (*get_text)(char *text, int len);
	void (*response_clicked)(void *data, Evas_Object *obj, void *event_info);
	int (*register_handler)(void *data);
	void (*unregister_handler)(void *data);
};
